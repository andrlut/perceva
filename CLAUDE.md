...
- **Gerenciar = uma tela por entidade, mesmo chassi** (2026-09-22): `/tasks` (práticas) e `/rewards-manage` (recompensas) são a superfície de CURADORIA — criar, adotar do catálogo (aba Sugeridas), editar (toque na linha), ordenar (alça à direita, `onPressIn={drag}` + `dragHitSlop` confinando o pan à coluna da alça + `resetDrag` via `onAnimValInit`), arquivar/restaurar/apagar (rodapé Arquivadas). Home e Vault são as superfícies de FAZER (concluir / comprar) e não têm botão flutuante de gerenciar: a Vault entra por `options-outline` na barra de topo, ao lado do pill dourado **Banco** (âncora do tour M4, `TourTarget id="rewards.bank"`). O catálogo de recompensas saiu da Vault ("Inspiração") e mora em Gerenciar › Sugeridas; o estado vazio da Vault aponta pra lá. Gotchas registrados na memória: `draggable-flatlist-reset` e `sheet-scroll-pressable`.
