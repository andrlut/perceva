...
- **Gerenciar = uma tela por entidade, mesmo chassi** (2026-09-22): `/tasks` (práticas) e `/rewards-manage` (recompensas) são a superfície de CURADORIA — criar, adotar do catálogo (aba Sugeridas), editar (toque na linha), ordenar (alça à direita, `onPressIn={drag}` + `dragHitSlop` confinando o pan à coluna da alça + `resetDrag` via `onAnimValInit`), arquivar/restaurar/apagar (rodapé Arquivadas). Home e Vault são as superfícies de FAZER (concluir / resgatar) e têm o mesmo par de flutuantes: calendário em cima, principal embaixo (Home → Todas as práticas; Vault → **Resgates**, dourado, `TourTarget id="rewards.log"`, âncora do M4) — Gerenciar mora DENTRO desse principal. O catálogo de recompensas saiu da Vault ("Inspiração") e mora em Gerenciar › Sugeridas; o estado vazio da Vault aponta pra lá. Gotchas registrados na memória: `draggable-flatlist-reset` e `sheet-scroll-pressable`.
- **Resgate = uso, sem Banco** (2026-09-23, migration `20260923000001`): recompensa segue a dinâmica das práticas. Resgatar na Vault paga e vale NA HORA (uma linha em `reward_redemption`, `used_at = redeemed_at`); não existe mais guardar pra depois, nem `use`/`sell`/`unuse`. Olhar pra trás é no **calendário** (frente Vault, `/history?front=vault`): o painel do dia lista os resgates com **desfazer** (`undo_reward_redemption`, estorna `cost_paid`) e tem **"Resgatar neste dia"** (`RedeemPickerSheet` → `BuyConfirmModal` → `redeem_reward_n` com `p_at` = meio-dia local do dia, paga com o saldo de HOJE). **Resgates** (`/rewards-history`) é o razão plano (segurar = desfazer) e hospeda Gerenciar. A celebração pós-resgate tem "Desfazer" (desfaz exatamente os ids criados). O feed do calendário voltou a 3 leituras (a 4ª, por `used_at`, duplicaria cada linha); `CalendarRedemption` perdeu `kind`. Telas/RPCs apagados: `rewards-bank`, VaultBankCard/ActionSheet, Sell/Use/UnuseConfirmModal. MCP `get_rewards` 0.4.6 sem `banked`.
